...
def compute_tables(df_in, report_source, outfile = "",central_platform = False):
    ...
    extra_vars = ["current_or_former_smoker","bmi_in_cat2","disease_duration_in_cat2","dmt_glucocorticoid","has_comorbidities","duration_treatment_cat1","duration_treatment_cat2","covid_wave1","covid_wave2"]
    #Feasability study
    df = df_in.copy()
    ...
    for extra_var in extra_vars:
        if extra_var in df:
            for strat_col in stratification_cols:
                if strat_col in df:
                    df.fillna("missing").groupby(["covid19_diagnosis","source"]+main_vars+[strat_col]+[extra_var]).count()[["secret_name"]].reset_index().to_csv(f"./results/{outfile}{report_source}_query3_{extra_var}_{strat_col}.csv")
                else:
